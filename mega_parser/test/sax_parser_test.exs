defmodule MegaParser.SaxParserTest do
  ...
  test "parse a0011" do
    ...
    assert output.accruals_teim == []
    assert output.accruals_heading_ssm == []
    assert output.accessrestrict_teim == ["This collection is open for research."]
    assert output.accessrestrict_heading_ssm == ["Information about Access"]
    assert output.appraisal_teim == []
    assert output.appraisal_heading_ssm == []
    assert output.arrangement_teim == []
    assert output.arrangement_heading_ssm == []
    assert output.bibliography_teim == []
    assert output.bibliography_heading_ssm == []
    assert output.bioghist_teim == []
    assert output.bioghist_heading_ssm == []
    assert output.custodhist_teim == []
    assert output.custodhist_heading_ssm == []
    assert output.fileplan_teim == []
    assert output.fileplan_heading_ssm == []
    assert output.note_teim == []
    assert output.note_heading_ssm == []
    assert output.odd_teim == []
    assert output.odd_heading_ssm == []
    assert output.originalsloc_teim == []
    assert output.originalsloc_heading_ssm == []
    assert output.otherfindaid_teim == []
    assert output.otherfindaid_heading_ssm == []
    assert output.phystech_teim == []
    assert output.phystech_heading_ssm == []
    assert output.prefercite_teim == ["Stanford University Student Life Photograph Album (A0011). Department of Special Collections and University Archives, Stanford University Libraries, Stanford, Calif."]
    assert output.prefercite_heading_ssm == ["Cite As"]
    assert output.processinfo_teim == []
    assert output.processinfo_heading_ssm == []
    assert output.relatedmaterial_teim == []
    assert output.relatedmaterial_heading_ssm == []
    assert output.scopecontent_teim == ["Images of students and student activities including track meets, women basketball players, dormitory rooms, picnics and off-campus excursions, commencement activities (including decoration of the arcades as parlors), and theatricals; there are also photographs of campus buildings before and after the 1906 earthquake and of Leland Stanford Jr.'s railroad track on campus. Non-Stanford images include Yosemite and scenes from a European trip."]
    assert output.scopecontent_heading_ssm == ["Scope and Contents"]
    assert output.separatedmaterial_teim == []
    assert output.separatedmaterial_heading_ssm == []
    assert output.userestrict_teim == ["All requests to reproduce, publish, quote from, or otherwise use collection materials must be submitted in writing to the Head of Special Collections and University Archives, Stanford University Libraries, Stanford, California 94304-6064. Consent is given on behalf of Special Collections as the owner of the physical items and is not intended to include or imply permission from the copyright owner. Such permission must be obtained from the copyright owner, heir(s) or assigns. See: http://library.stanford.edu/depts/spc/pubserv/permissions.html.", "Restrictions also apply to digital representations of the original materials. Use of digital files is restricted to research and educational purposes."]
    assert output.userestrict_heading_ssm == ["Ownership & Copyright"]
