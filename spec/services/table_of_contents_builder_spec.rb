...
RSpec.describe TableOfContentsBuilder do
  ...
  context "when rendering a series whose children has online content" do
    it "renders a grayed out online icon" do
      document = SolrDocument.find("MC148_c00002")
      output = described_class.build(document)
      toc_hash = JSON.parse(output)
  ...
  context "when rendering a child which has direct online content" do
    it "renders a green icon" do
      document = SolrDocument.find("MC148_c07360")
      output = described_class.build(document)
      toc_hash = JSON.parse(output)

      online_component = toc_hash[1]["children"][0]["children"][0]
      expect(online_component["text"]).to include "online-direct-content"
    end
  end
end
