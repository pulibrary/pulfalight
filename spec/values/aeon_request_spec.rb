...
RSpec.describe AeonRequest do
  ...
  describe "SolrDocument#aeon_request" do
    ...
    context "when there's no access note" do
      let(:fixture_path) do
        Rails.root.join("spec", "fixtures", "aspace", "generated", "cotsen", "COTSEN4.EAD.xml")
      end
      it "renders" do
        result = indexer.map_record(record)
        component = find_component(component_id: "ref53", record: result)
        document = SolrDocument.new(component)
    ...
    context "when an access note has apostrophes and commas" do 
      let(:fixture_path) do
        Rails.root.join("spec", "fixtures", "aspace", "generated", "mss", "C1491.processed.EAD.xml")
      end
      it "strips them out" do 
        ...
        request = document.aeon_request
        request_id = request.form_attributes[:Request]
        expect(request.form_attributes[:"ItemInfo1_#{request_id}"]).to eq "Livingston's Battalion: Accessrestrict with hardcoded apostrophe"
      end 
    end 
  ...
  context "when it's a component with rcpph mapping" do
    ...
    it "sets mudd as the site, leaves ReCAP as the location, and adds a delivery note" do
      result = indexer.map_record(record)
