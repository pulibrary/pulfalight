...
describe "viewing catalog records", type: :feature, js: true do
  context "when viewing a component show page" do
    ...
    it "has a suggest a correction form", js: false do
      ...
      expect(page).to have_field "suggest_a_correction_form_context", visible: false, type: :hidden, with: "#{Capybara.app_host || Capybara.default_host}/catalog/MC221_c0059"
    end
    ...
    it "has a component level storage note" do
      visit "catalog/MC221_c0059"
      expect(page).to have_css("dd.blacklight-summary_storage_note_ssm", text: "Mudd Manuscript Library (mudd)\nBox 3")
    end
  ...
  context "when given something with access restrictions", js: false do
    ...
    it "will truncate accessrestrict text longer than 300 characters, create a button link that, when clicked opens a modal with the full accessrestrict text" do
      visit "/catalog/C0187_c00001"
      expect(page).to have_css(".alert-heading", text: "Access Restrictions")
      expect(page).to have_css("svg title", text: "Icon Restrict")
      expect(page).to have_css(".access-restrict-warning", text: /^.{0,300}$/)
    ...
    it "displays a warning above the description that items may be restricted if review is required" do
      ...
      expect(page).to have_css(".access-review-warning", text: /^.{0,300}$/)
      expect(page).to have_button "Read full Conditions Governing Access"
      find("#restrictionsModal-trigger-link").click
    ...
    it "will not truncate accessrestrict text less than 300 characters" do
      visit "/catalog/AC198-06_c1"
      expect(page).to have_css(".alert-heading", text: "Access Restrictions May Apply")
      expect(page).not_to have_css(".access-restrict-warning")
      expect(page).not_to have_button "Read full Conditions Governing Access"
      expect(page).not_to have_css("#restrictionsModal")
    end
  end
  ...
  context "with a collection show page" do
    ...
    it "has a suggest a correction form", js: false do
      ...
      expect(page).to have_field "suggest_a_correction_form_context", visible: false, type: :hidden, with: "#{Capybara.app_host || Capybara.default_host}/catalog/MC221"
      expect(page).to have_field "suggest_a_correction_form_location_code", visible: false, type: :hidden, with: "publicpolicy"
    end
  ...
  describe "notes", js: false do
    context "on a collection page" do
      ...
      it "displays consecutive boxes as a range" do
        visit "catalog/C1643"
        expect(page).to have_content("Storage Note:")
        expect(page).to have_content("Firestone Library (mss)Boxes B-001494-B-001544; P-000146")
      end
      it "displays multiple storage locations as a list" do
        ...
        expect(page).to have_selector("dl", text: "Firestone Library (scamss)Boxes B-001615-B-001618; P-000162")
  ...
  describe "record that has bioghist notes for persnames", js: true do
    it "displays more information in a modal" do
      ...
      click_button "(Historical Information)", match: :first
