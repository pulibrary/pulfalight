...
describe "controller requests", type: :request do
  ...
  context "when an AJAX request is transmitted for a collection document" do
    ...
    let(:document_id) { "WC064" }

    it "renders a minimal HTML template in the response" do
      headers = { "X-Requested-With" => "XMLHttpRequest" }
      get("/catalog/WC064", headers: headers)
      expect(response.body).to include("<div id=\"document-minimal-#{document_id}\"")
      html_tree = Nokogiri::HTML(response.body)
      field_name_elements = html_tree.css("#document-minimal-#{document_id} .document-minimal-field-name")
      ...
      field_value_elements = html_tree.css("#document-minimal-#{document_id} .document-minimal-field-value")
